...
- **Index-time (per chunk).** Orthographic normalization is a pure O(n) pass over chunk text (regex strip + codepoint map + NFC) — microseconds, dwarfed by the embedding API call that already dominates indexing. The async `Normalizer` is effectively synchronous for the pure-TS default (returns a string), so awaiting adds nothing in Approach A.
- **Migration is cheap by construction.** Because `content_normalized` is a *plain nullable* column, `ALTER TABLE … ADD COLUMN content_normalized TEXT` is a metadata-only, instant operation — unlike a `GENERATED ALWAYS AS … STORED` column, which would rewrite the whole table under lock. (A concrete edge of the library-owned placement over the SQL/generated-column alternative.) Build the new GIN trigram index with `CREATE INDEX CONCURRENTLY` and drop the old one after, to avoid a long write lock. The one-time backfill (re-normalize `content` → `content_normalized` and re-derive the tsvector for Arabic rows) is an app-side batch job — same shape/cost as a re-embed; run it in **bounded batches** to avoid WAL spikes, long transactions, and table bloat. Sequence: add column → backfill in batches → build index concurrently → switch leg SQL to `content_normalized` → drop old index.
- **Query-time.** Query normalization is one cheap transform over a short string. The lexical legs run against `content_normalized` with the **same GIN index type and plan shape** as before — no added cost, just a different column. `arabic` Snowball stemming runs inside `to_tsquery` at query time (negligible); the document tsvector is precomputed by the trigger, so the FTS leg gains no per-query cost. Stemming broadens recall, but `candidateLimit` already caps each leg's output, so fusion/rerank input size is unchanged. The 3 legs still run in parallel — no new leg, no added latency. Stop-word normalization runs once per cache refresh (`CachingStopWordsLoader`, 30s TTL) over a small set.
- **Storage.** `content_normalized` is a second copy of chunk text. Arabic normalization is roughly length-preserving (drops a few diacritics, maps characters 1:1), so the text storage/WAL for that column roughly doubles — small relative to the embedding vectors.
- **Future (Phase 2) costs to budget for, not incurred now:**
  - **Embedding dimension** is configurable now; switching 384 → 1024 (bge-m3) later ~2.7×'s vector storage, enlarges the IVFFlat index and its RAM footprint, and makes each distance computation costlier — revisit `lists`/`ivfflatProbes` (or move to the vchordrq index) when that lands.
  - **External `Normalizer` service** turns each index/query normalization into a network call: batch document normalization (already a batch loop in `RagIndexer`), keep the query-side call lean (cache, co-locate the service), and set a timeout with **graceful fallback to raw text** so a slow service can't stall search.
  - **Always-on reranking** is the dominant query-time cost when enabled — a cross-encoder pass over up to `candidateLimit` docs. That is why the budget profile reranks only top-N and the reranker stays opt-in.
