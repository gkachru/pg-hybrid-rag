...
| `src/recommend.ts` | Per-language setup recommender (`recommendForLanguage`) — embedder/dimensions/vectorMinScore + stemming/normalization/CJK flags |
...
- **Per-language setup recommender** — `recommendForLanguage(lang)` (`src/recommend.ts`) is a pure, advisory lookup returning calibrated starting points (`bge-m3`/1024-dim/`maxTokens` 8192/`vectorMinScore` 0.4 — measured for ar/th, multilingual default elsewhere; `maxTokens` is a truncation ceiling, NOT a recommended chunk size — chunk well under it) plus structural flags derived from existing sources of truth: `stemming` mirrors `rag_fts_config` (sync-guarded by a test against `sql/014`, and includes Arabic which `BM25_LANGUAGE_GROUPS` omits), `isCjk` from the CJK set, `needsNormalization` from `normalizeForLanguage`. No segmenter is recommended (Thai A/B was a clean negative). Embedding `dimensions` is a per-database choice (one `vector(N)` column shared by all tenants), so mixed-dimension models need separate databases. Constants stay module-internal (tests import them via relative path), matching the `bm25LanguageGroups` precedent.
